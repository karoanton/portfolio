"""
Linguistic Projects:
Language Modeling: train a language model on a single text corpus
Corpus Analysis: analyze linguistic features (frequency analysis, n-gram analysis, POS tagging) in a text corpus
Cipher: Create a cipher to deliver secret messages with

Hobby Projects:
Crochet Stash Buster Project Generator: Input amount of yarn (yardage/meterage or weight) and yarn weight (thickness) and output list of projects that can be completed with that amount of yarn.
Elevated Version: includes skill level, colourwork (# of colours), etc.
"""
